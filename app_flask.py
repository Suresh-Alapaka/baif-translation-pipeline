...
def get_mms(tgt):
    if tgt not in mms_cache:
        ...
        mms_cache[tgt] = {
            "model":     VitsModel.from_pretrained(MMS_MODELS[tgt], local_files_only=True),
            "tokenizer": AutoTokenizer.from_pretrained(MMS_MODELS[tgt], local_files_only=True),
        }
...
def get_translator(src, tgt):
    ...
    if key not in trans_cache:
        ...
        mdl = AutoModelForSeq2SeqLM.from_pretrained(ckpt,trust_remote_code=True,local_files_only=True)
...
def synthesize_segments(segments, texts, tgt, out_path,
    ...
    inside its original Whisper timestamp window when it runs long,
    then place it sequentially on a silent track — translated voice
    only, no original background audio.

    Speed-up is capped (MAX_SPEEDUP) so voices don't sound unnatural.
    If a segment is still too long after the cap, it's shifted to
    start right after the previous segment ends, so segments never
    overlap each other — but sync with the video may drift slightly
    in that case.
    """
    MAX_SPEEDUP = 1.6  # cap so sped-up speech still sounds natural
    ...
    cursor_ms = 0  # end time (ms) of the last placed segment
    ...
    for seg, txt in zip(segments, texts):
        ...
        # Place at the original timestamp unless the previous segment
        # (after speed-up) is still playing — then start right after it
        start_ms = max(int(seg["start"] * 1000), cursor_ms)
        if start_ms > int(seg["start"] * 1000):
            note += " (shifted, still ran long)"
    ...
    final_len = max(int(total_duration_sec * 1000), cursor_ms)
...
<div class="header">
  ...
  <button id="restartBtn" onclick="restartServer()"
    ...
    🔄 Restart Server (switch direction)
  </button>
...
// Restart button - tells the launcher (port 5050) to stop and
// relaunch this app_flask.py process, then reloads this page once
// it's confirmed back up. Useful to fully clear memory/state when
// switching translation direction on a resource-constrained machine.
async function restartServer() {
  ...
  try {
    const res = await fetch('http://127.0.0.1:5050/restart-script', { method: 'POST' });
    const data = await res.json();
    if (data.success) {
      ...
      btn.disabled = false;
      btn.textContent = '🔄 Restart Server (switch direction)';
    }
  } catch (err) {
    status.textContent = 'Could not reach launcher (is it running on port 5050?): ' + err.message;
    status.style.color = '#ffcdd2';
    btn.disabled = false;
    btn.textContent = '🔄 Restart Server (switch direction)';
  }
