...
def get_translator(src, tgt):
    ...
    if key not in trans_cache:
        ...
        tok = AutoTokenizer.from_pretrained(ckpt, trust_remote_code=True,
                                             local_files_only=True, cache_dir=MODELS_DIR)
...
def synthesize_segments(segments, texts, tgt, out_path,
    ...
    for idx, (seg, txt) in enumerate(zip(segments, texts)):
        ...
        overlap_ms = len(seg_audio) - target_ms
        if overlap_ms > 0:
            note += f" (ran {overlap_ms}ms over, may overlap next line)"
  ...
  <div class="panel right">
    ...
    <div id="downloadSection" style="display:none">
      ...
      <div class="downloads">
        ...
        <a id="dlSRT" class="dl-btn srt" href="#" target="_blank" rel="noopener">⬇️ SRT Subtitles</a>
        <a id="dlVTT" class="dl-btn vtt" href="#" target="_blank" rel="noopener">⬇️ VTT Subtitles</a>
      </div>
    </div>
  </div>
</div>
