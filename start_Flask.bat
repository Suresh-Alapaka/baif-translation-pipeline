@echo off
REM ---------------------------------------------------------
REM Activates whisper-env, runs app_flask.py in the
REM background, waits until it's actually ready, then opens
REM the app page in your default browser automatically.
REM
...
REM Start the app in a separate, minimized window so this
REM window can move on to waiting/opening the browser.
start "App" /min python "%SCRIPT_PATH%"

REM Poll until port 5000 is actually accepting connections
